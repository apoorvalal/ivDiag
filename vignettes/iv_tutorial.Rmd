...
Users can also adjust the look of the plot and control which estimates to shown using various options. For example:
```{r}
...
Setting `0.178` as the prior mean and `0.137` as its standard deviation (SD), we can perform a LTZ adjustment on the IV estimate. 
```{r}
